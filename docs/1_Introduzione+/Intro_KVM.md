...
- **Consolidamento delle risorse**: il dimensionamento di un server che deve erogare un servizio o eseguire
...
- **Backup**: la virtualizzazione permette di creare backup e snapshot di tutta la nostra infrastruttura
...
- **Deployment**: ci sono soluzioni come Red Hat Satellite e Ansible che rendono il deployment
...
- **Partitioning**: La CPU è divisa in più parti e ognuna lavora individualmente in maniera isolata e può
eseguire un sistem opartivo diverso (come le IBM Logical Partitions - LPARs)
- **Full Virtualization**: L'hardware della macchina virtuale è emulato in maniera trasparente, così che
...
- **Paravirtualization**: La macchina *guest* esegue una versione modificata del sistema operativo e dei
...
- **Type 1 Hypervisor**: Il software dell'hypervisor viene eseguito direttamente sulla macchina fisica (in altre
...
- **Type 2 Hypervisor**: La macchina *host* ha un sistema operativo sul quale viene eseguito un software di
